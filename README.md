...
- `SpeelosMenu.csproj` - references game DLLs from the Steam install and BepInEx from the Thunderstore Mod Manager profile.
...
- `SkBepInExLoader` is the BepInEx plugin (GUID `com.speelo.speelosmenu`). It binds config, then `SkLoader` creates the `SpeelosMenu` GameObject with the controllers.
