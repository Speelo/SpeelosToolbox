...
- **Player** - grouped into Character, Cheats and Info. Bookmarks save a spot to teleport back to, and loadouts snapshot your whole inventory to restore later. One-click food, healing, repair, skills, puke, inventory size and stats; toggles for god mode, flying, no-cost building, build anywhere, infinite stamina and taming; status effects; and readouts for position, portals, coordinates and nearby enemies. Toggles light up with a green outline while they are on, read from live game state rather than remembered, so they stay correct even when you use Valheim's own hotkeys.
